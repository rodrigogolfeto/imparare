...
$hash_code1 = strip_tags(trim($_POST['hash_code1']));
$hash_code2 = strip_tags(trim($_POST['hash_code2']));

if($hash_code1 == md5('7') && $hash_code2 == md5('9')){
    $text_menu_id = '7';
    $text_subm_id = '9';
    $nome_curso = 'I Corsi di Portoghese';
}elseif($hash_code1 == md5('10') && $hash_code2 == md5('12')){
    $text_menu_id = '10';
    $text_subm_id = '12';
    $nome_curso = 'Traduzioni Portoghese';
}else{
    $resultado = array('code'=>400,'message'=>'Preencha todos os campos.','code'=>false);
...
if($validacao==true && $action=='send-sottoscrizione'){

    $sql = "INSERT INTO tag3_textos SET
    
    text_menu_id = '$text_menu_id',
    text_subm_id = '$text_subm_id',
    text_titulo = '$nome',
    text_info1 = '$email',
    text_texto = '$mensagem',

    text_data = DATE(NOW()),text_data_publicacao = NOW(),text_hora1 = '00:00:00',text_hora2 = '00:00:00',text_usu_id = '0'";
    ...
    if($cid){
        ...
        try {
            ...
            $contato_titulo		= 'Inscrição #'.time();
            $contato_subtit		= 'Segue os detalhes da inscrição.';
            $contato_imagem		= $config['urlSite'] . 'images/topo-email.png';//400x100
            
            $contato_cor_borda	= '#88BC59';
            $contato_txt_rodape	= 'Esta mensagem foi enviada através do formulário de inscrição';
